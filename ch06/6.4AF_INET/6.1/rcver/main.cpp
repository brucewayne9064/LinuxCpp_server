...
int main()
{
	int sockfd,size,ret;
	char on = 1;
	struct sockaddr_in saddr; //本地地址
	struct sockaddr_in raddr;  //对面地址
	...
	while (1)
	{
		...
		ias.s_addr = iph.m_uiSourIp;  //源ip
		iad.s_addr = iph.m_uiDestIp;  //目的ip


		printf("(sIp=%s,sPort=%d), \n(dIp=%s,dPort=%d)\n", 
                inet_ntoa(ias), ntohs(udph.m_usSourPort),   
                inet_ntoa(iad), ntohs(udph.m_usDestPort));
		printf("recv data:%s\n", rbuf + 28);
	}
