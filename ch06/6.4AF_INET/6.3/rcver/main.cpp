...
int main(int argc, char **argv) {
	...
	while (1) {
        ...
        //检查数据包是否为 IPv4 协议并且没有选项字段的
		if (*iphead == 0x45) { /* Double check for IPv4
							  * and no options present */
							  //printf("Layer-4 protocol %d,", iphead[9]);
			memcpy(&iph, iphead, 20);
            //检查数据包的源 IP 地址和目的 IP 地址是否相同，如果相同则跳过
			if (iphead[12] == iphead[16] && iphead[13] == iphead[17] && iphead[14] == iphead[18] && iphead[15] == iphead[19])
				continue;
            //源 IP 地址是否为 127.0.0.1,是则跳过，127.0.0.1 是一个特殊的 IP 地址，
            //它表示本地回环地址。这个地址用于在本地计算机上进行网络通信测试，
            //数据包发送到这个地址会被立即返回而不会被发送到网络上
			if (iphead[12] == 127)
				continue;
			printf("-----cn=%ld-----\n", cn++);
			printf("%d bytes read\n", n);
