...
<img src="图片/tinyhttp.drawio.png" alt="tinyhttp.drawio" style="zoom:80%;" />

## Ch10：基于Libevent的FTP服务器

### 一、Reactor模式

Reactor模式是一种基于**事件驱动**的设计模式，应用程序需要提供相应的接口并注册到reactor上，如果相应事件发生，reactor将主动调用应用程序注册的接口，这些接口叫做**回调函数**。可以通过Libevent框架实现。

Reactor模式有三种常见的实现方式：

- 单Reactor单线程：只有一个线程负责监听、接收和处理所有的请求事件，优点是模型简单，缺点是性能和可靠性较差。
- 单Reactor多线程：有一个线程负责监听和接收请求事件，然后将事件分发给多个线程进行处理，优点是可以利用多核CPU的性能，缺点是Reactor线程可能成为性能瓶颈。
- 主从Reactor多线程：有一个主Reactor线程负责监听和接收连接事件，然后将连接分配给多个从Reactor线程，从Reactor线程负责监听和分发读写事件，优点是可以避免单个Reactor线程的性能瓶颈，缺点是模型复杂度较高。

reactor模式框架：

- 事件源（handle）：可以产生事件的对象，例如客户端的连接、读写请求等。事件源需要被注册到事件多路分发器上，才能被检测和处理。
- 同步事件分离器（Synchronous event demultiplexer）：通常是由操作系统提供的I/O多路复用的机制，例如select, epoll。程序首先将handle（事件源）以及对应的事件注册到event demultiplexer上；当有事件到达时，event demultiplexer就会发出通知，通知Reactor调用事件处理程序进行处理。
- 初始分发器（Initiation Dispatcher） / reactor管理器：事件管理的接口，内部使用event demultiplexer注册，注销事件；并运行事件循环，当有事件进入"就绪"状态时，调用注册事件的回调函数（事件处理器）处理事件。
- 事件处理器（event handler）：事件处理程序提供了一组接口，在Reactor相应的事件发生时调用，执行相应的事件处理，通常会绑定一个有效的handle。
- 具体事件处理器（Concrete Event Handler）：就是事件处理器接口的实现，实现了应用程序的某一个服务，当事件发生时，对应的处理机制。

reactor事件处理流程：

![20191210090959321](图片/20191210090959321.png)

### 四、FTP协议

**4.1FTP概述**

FTP协议用于从一台主机传送文件到另一台主机的协议。对于一个客户端，需要和服务器建立两个连接，一个是控制连接，一个是数据连接。控制连接用于客户端和服务器之间的命令和响应的传递，数据连接用于传输数据信息。

**4.2FTP的传输方式**

- ASCII传输方式：用来传输文本文件，它会将文件中的每个字符转换成ASCII码，并且在每行的末尾添加回车和换行符
- 二进制传输方式：传输非文本文件的，比如图片，音频，视频等。它会将文件中的每个字节原样发送，不做任何转换

**4.3FTP的工作方式**

主动与被动是相对于服务器而言的，即服务器是主动连接还是被动连接。主动方式可以避免 服务器端防火墙的干扰，被动方式可以避免客户端防火墙的干扰。

- 主动方式（PORT）：客户端开启一个端口（大于1024）与服务器21端口建立**控制连接**。客户端通过控制连接上的PORT命令向服务器发送ip和端口，让**服务器来连接该ip和端口**，建立**数据连接**。
- 被动方式（PASV）：客户端开启一个端口（大于1024）与服务器21端口建立**控制连接**。客户端通过控制连接向服务器发送PASV命令，服务器开启一个端口（大于1024），将ip和端口号发给客户端，让**客户端来连接该ip和端口**，建立**数据连接**。

**4.4FTP命令**

FTP命令主要用于控制连接，根据功能分为访问控制命令，传输参数命令，FTP服务命令。

**4.5FTP应答码**

服务器在执行完相关命令后，返回给客户端的执行结果信息，客户端可以通过应答码及时了解服务器当前的工作状态。

### 五、开发FTP服务器

FTP服务器采用Libevent高性能事件通知库，基于C++11的线程池。使用并发模型实现多个客户端同时相连。

**并发模型分为三大类：**

- 多进程模型：每接受一个连接就fork一个子进程
- 多线程模型：每接受一个连接就create一个子线程（Ch9的`tinyHttp`就是基于这种做法）
- 事件驱动模型：基于epoll等io多路复用，程序的基本结构是时间循环结合非阻塞io，以事件回调的方式实现业务逻辑。结合线程池，避免频繁创建和销毁线程，让空闲的线程可以重新执行任务。

**服务器端：**

线程池基于C++11的socket API。主线程创建线程池，线程池初始化多个线程，当有连接到来时，分发一个线程去处理连接。

事件监听基于Libevent API，该库提供了包括创建事件（绑定base和回调函数），开启事件分发，销毁事件和base等函数api。主线程将监听事件加入base，开启事件循环，当有新的连接到来时，base调用回调函数处理新连接，此处新的连接指的是控制连接。

新线程创建一个命令处理器，命令处理器处理和FTP协议有关的指令，包括登录，登记客户端信息，目录操作，上传文件，下载文件。

线程创建一个base用于读写事件循环。

ftp指令中涉及到套接字缓冲区读写以及错误异常的被设置为缓冲区事件，并创建读，写，异常三个回调函数，注册到线程base。当这些事件发生时，会调用相应的回调函数，回调函数又会调用命令处理器去处理指令。

**客户端：**

客户端需求分析：

- 易于操作的图形界面，方便用户登录，上传和下载等操作
- 完善的功能，包括登录，退出，目录操作，文件下载上传，目录下载上传，文件目录的删除，断点续传，传输状态反馈
- 稳定性高

客户端使用wininet api在Windows端开发。
